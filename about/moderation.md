---
id: IHMR-PAGE-003
type: page
title: Comment Moderation
status: current
created: 2026-09-03
updated: 2026-09-03
ai_disclosure: substantially-ai-assisted
ai_tools:
  - claude-opus-5
signed_off_by:
signed_off_date:
license: CC-BY-4.0
lang: en
---

# Comment Moderation

**Every comment on [projectihmr.com](https://projectihmr.com) is read by a person before it is
published.** If we do not publish yours, we write back and tell you why.

We want to explain why we do it this way, because "all comments are moderated" can sound like a
polite way of saying "we only publish people who agree with us." That is not what this is.

## Why we moderate

- **Disagreement is the point.** We need correction more than agreement, and a critical comment
  is more useful to us than a supportive one. Being wrong in public is how this project improves.
- **But personal health information should not end up on a public website**, whether it is yours
  or someone else's - and people share it without meaning to, especially on a health project.
- **And medical misinformation attached to our name causes real harm.** We would rather be slow
  than be a distribution channel for it.

So: a human reads everything, and nothing appears until they do.

## How it works

1. You write a comment and verify your email address once.
2. Your comment goes into a queue. It is not visible to anyone yet.
3. A person reads it - usually within a day or two.
4. It is either published, or you get an email explaining why not.

Your email address is never shown publicly, never shared, and never appears in our API, feeds,
or anywhere else. You can comment under your name or anonymously.

## When we do not publish something

We use a fixed set of reasons, so that a decision is never arbitrary and so we can review our
own patterns:

| Reason | What it means |
|---|---|
| Off topic | Not about the document it was left on |
| Not substantive | No argument, evidence, or question we can act on |
| Personal health information | Contains identifiable health information |
| Identifies someone else | Names a person who has not agreed to it |
| Medical advice | Offers or asks for clinical advice |
| Unsourced claim | States a factual claim that needs a source and gives none |
| Abusive | Harassment, abuse, or targeting a person |
| Spam or promotional | Advertising or link-farming |
| Duplicate | Already made and answered |
| Legal risk | Defamatory, confidential, or otherwise creates real legal exposure |

**"We disagree with it" is not on this list, and never will be.**

If your comment is not published, you will get the reason and, where it would help, a note on
what could change. **You are welcome to revise it and try again** - that is not a strike against
you, and most rejections are fixable in a sentence or two.

## If you think we got it wrong

Email hello@projectihmr.com and say so. We will look again, and a person will reply.

## What we publish about moderation

We publish aggregate numbers - how many comments we received, how many we published, and the
reasons for the rest. Individual decisions stay private; the pattern does not.

A project built on working in the open should not have a part of it you cannot see into.
